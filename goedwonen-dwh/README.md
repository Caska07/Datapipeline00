...
- `adf/linkedService/LS_AzureSqlDWH.json` – placeholder linked service voor de OLAP/DWH-database
- `adf/linkedService/LS_AzureSqlOLTP.json` – placeholder linked service voor de OLTP-database
  (zelfde `Data Source`/server als DWH, andere `Initial Catalog`)
- `adf/linkedService/LS_OpenMeteo_REST.json` – gratis Open-Meteo historical archive API (geen key)
- `adf/dataset/DS_OLTP_*.json` / `DS_Staging_*.json` – 8 tabelparen (ADRES, OBJECTEN,
  COMPLEX, WERKSOORT, PROJECT, PROJECTREGEL, WERKVERZOEK, WERKOPDRACHT), bron resp. staging
...
2. Connectiegegevens invullen in `LS_AzureSqlDWH.json` (OLAP) en
   `LS_AzureSqlOLTP.json` (OLTP) — zelfde server, andere database-naam,
   en dezelfde integration runtime als je VPN-gekoppelde omgeving vereist
3. Open-Meteo geeft windsnelheid in km/h, niet Beaufort — reken dit om in
   `usp_Load_DimWeer` of al bij binnenkomst in staging
4. Check of `RegioLatitude`/`RegioLongitude` (Heerlen) precies genoeg is
